...
- **Repost-chain heuristic.** When a new `external_id` first appears, it joins an existing job iff there's a prior listing for the **same company** with the same normalized `match_key` (`title|location|department`) that has since **disappeared** (last seen before today) within `REPOST_WINDOW_DAYS` (30); otherwise a fresh job is created. A repost requires the old posting to have gone away first, so concurrent duplicate requisitions of the same role become **separate jobs**, not reposts. A job's age always traces to `MIN(first_seen_on)` and is never reset by a relist.
