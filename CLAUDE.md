...
- **poller/** — TypeScript Node worker. One adapter per ATS, a per-run orchestrator, repost linking, and normalization. Runs as a one-shot `npm run poll`, writes a `poll_runs` row, and `process.exit(0/1)`. No HTTP surface. Deployed as a Railway daily cron.
...
- **Jobs vs listings.** A `job` is the logical role (stable id across relists); a `listing` is one individual posting seen on a feed. The poller's write model is `listings`; `jobs` is a surrogate-identity table so a job id resolves *before* its listing is inserted.
...
- **`is_open` is derived**, not stored: a listing is open ⇔ `closed_on IS NULL` (the poller always sets `closed_on`/`last_seen_on` together). The view computes `is_open`; a partial index serves open-only reads.
- **UTC everywhere.** The poller's "today" and the DB's `current_date` (age math + close-out) must agree — both pinned to UTC.
- **Railway daily cron.** The poller is a Railway service purely because `deploy.cronSchedule` is set; it boots, runs one poll, and exits. `restartPolicyType: "NEVER"`.
...
- `.claude/rules/code-style.md` — TypeScript, JSDoc, import ordering, naming, error handling. Loads for `client/**/*.{ts,tsx}`, `server/**/*.ts`, and `poller/**/*.ts`.
